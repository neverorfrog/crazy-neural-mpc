...
class CrazyflieServer(Node):
    ...
    def _cmd_vel_legacy_changed(self, msg, uri=""):
        ...
        """
        self.get_logger().info(f"{uri}: Received vel setpoint topic")
        roll = msg.angular.x
        pitch = msg.angular.y
        yawrate = msg.angular.z
        thrust = int(min(max(msg.linear.z, 0, 0), 60000))
        self.get_logger().info(f"{uri}: {roll} {pitch} {yawrate} {thrust}")
        self.swarm._cfs[uri].cf.commander.send_setpoint(
            roll, pitch, yawrate, thrust)

    def _cmd_attitude_setpoint_changed(self, msg, uri=""):
        self.get_logger().info(f"{uri}: Received attitude setpoint topic")
        self.swarm._cfs[uri].cf.commander.send_setpoint(
