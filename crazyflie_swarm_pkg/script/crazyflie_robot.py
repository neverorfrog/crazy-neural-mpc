...
class CrazyflieRobot:
  ...
  def take_off(self, absolute_height=None, duration=None):
    ...
    if absolute_height is None: absolute_height = self.default_take_off_height
    if duration is None: duration = self.default_take_off_duration
  ...
  def land(self, duration=None):     
    self.is_flying = False
    if duration is None: duration = self.default_land_duration   
